...
export function scoreAndClassify(lead: Lead, event: EventInfo | undefined): { changedPriority: boolean; reclassified: boolean } {
  ...
  // ready_score ranks HOT leads for the Daily Call List — 0 for non-HOT or ineligible
  // isReadyEligible() excludes content/pricing pages, platform domains, low-confidence sites
  lead.ready_score = (priority === "HOT" && isReadyEligible(lead)) ? computeReadyScore(lead) : 0;
